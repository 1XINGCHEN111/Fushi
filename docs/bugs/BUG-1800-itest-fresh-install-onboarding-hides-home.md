...
- **[x] ② 已加自动化测试** — `home_onboarding_launch_gate_test.dart` 及 iPhone `feature_flows_test.dart` GREEN；RED 时导航目标实际为 0、exit 1。完成前审查后，`navigation_stability_test.dart` 另加入真实“设置 → 系统 → 重新打开新手引导 → 系统返回”深层路由，纳入 iOS/macOS 零跳过设备门（提交 `a3e82e646`），避免测试隔离变成向导 UI 盲区。
