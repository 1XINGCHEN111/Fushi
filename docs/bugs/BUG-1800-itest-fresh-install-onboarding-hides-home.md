...
- **[x] ② 已加自动化测试** — `home_onboarding_launch_gate_test.dart` 及 iPhone `feature_flows_test.dart` GREEN；RED 时导航目标实际为 0、exit 1。完成前审查后，`navigation_stability_test.dart` 另加入真实“设置 → 系统 → 重新打开新手引导 → 系统返回”深层路由，纳入 iOS/macOS 零跳过设备门（提交 `a3e82e646`），避免测试隔离变成向导 UI 盲区。提交 `0e7caed50` 再新增独立 clean-install gate：只有该用例用 `FUSHI_TEST_ALLOW_STARTUP_ONBOARDING=true` 启用无旁路首启，断言自动 push、pending 标志为 false、焦点 Skip、关闭后 `onboardingCompleted=true`；独立 Mac 数据根 5 秒绿，iPhone SE 物理机 14 秒绿。
