...
- `ingame_lookup_geometry`：`implemented_unverified` — In-game dictionary lookup is implemented but unverified: the KiriKiri sensor supplies per-glyph geometry and paints the selection highlight, while Hibiki reuses its existing popup in a dedicated off-screen galCard WebView2 surface, captures BGRA, and presents that bitmap through the v14 shared-memory frame route into a game Layer. Embedded mode suppresses only the popup sentence banner; dictionary, audio, mining, theme, and nested-card rendering remain shared with the normal Fushi popup. Coordinate conversion, KAG message-layer identity, submit/hover fencing, and off-screen resize handling have source-level guards, but the required automated verification was skipped for this update and no fresh same-session bitmap-card E2E has been recorded.
- codepage：game-specific
...
- In-game dictionary lookup remains implemented_unverified. The production route reuses the Fushi popup in an off-screen galCard WebView2, captures a bounded BGRA frame, and displays it through the v14 KiriKiri Layer route; the game-side sensor still owns glyph hit-testing and selection highlighting. The geometry sensor is gated on a third-party textrender.dll plus a runtime probe for global.TextRender.getCharacters, so it does not generalise to KiriKiri as an engine. No fresh same-session bitmap-card E2E or required offline verification has been recorded.
