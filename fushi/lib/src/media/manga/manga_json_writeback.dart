...
/// 把 [mangaJsonPath] 第 [pageIndex] 页上属于 [region] 的旧块换成 [blocks] 并原子
/// 落盘，**返回落盘后的完整 payload**。
///
...
/// 也不应再读一次。
///
/// - [region] / [blocks] 都是页图像素坐标；「属于区域」与 z_index 重编号的规则见
///   `replaceMangaPageRegion`（区域外块保序在前、新块在后）。
/// - 既有 `ocr` 元数据（引擎/签名/schema 版本）原样保留——抹掉它会让
///   `manga_ocr_cache_recovery.dart` 的同源判定失效，整卷缓存被判为异源作废。
/// - 页越界 / 文件缺失 / JSON 无该页 → [StateError]。
Future<MokuroPayload> replaceMangaBlocksInRegion({
  ...
  return runExclusiveOnMangaJson<MokuroPayload>(mangaJsonPath, () async {
    ...
    images[pageIndex] =
        replaceMangaPageRegion(payload.images[pageIndex], region, blocks);
