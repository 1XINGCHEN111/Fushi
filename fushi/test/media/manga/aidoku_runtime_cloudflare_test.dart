...
void main() {
  ...
  test('every invoke carries the shared identity and current cookies',
      () async {
    await jar.replaceForHost('mangafire.to', const <AidokuCookie>[
      AidokuCookie(name: 'cf_clearance', value: 'seed', domain: 'mangafire.to'),
    ]);
    installHost((_) async => <String, Object?>{
  ...
  test('challenge → resolver solves → retried once with the new cookie',
      () async {
    final List<Uri> resolved = <Uri>[];
    int calls = 0;
    installHost((Map<Object?, Object?> request) async {
      calls++;
      if (calls == 1) throw challenge('https://mangafire.to/filter?keyword=x');
    ...
    expect(resolved.single.toString(), 'https://mangafire.to/filter?keyword=x');
  ...
  test('resolver declines → original challenge error surfaces, no retry',
      () async {
    installHost((_) async => throw challenge('https://mangafire.to/'));
    final IosAidokuRuntime runtime = IosAidokuRuntime(
      jar: jar,
      resolver: (_) async => false,
    );

    await expectLater(
      runtime.search('/pkg.aix', query: 'x'),
      throwsA(isA<AidokuRuntimeException>()
          .having((AidokuRuntimeException e) => e.code, 'code',
              kAidokuCloudflareChallengeCode)
          .having((AidokuRuntimeException e) => e.challengeUrl.toString(),
  ...
  test('a second challenge after solving is final — never loops', () async {
    ...
    await expectLater(
      runtime.search('/pkg.aix', query: 'x'),
      throwsA(isA<AidokuRuntimeException>().having(
  ...
  test('falls back to the global gate when no resolver is injected', () async {
    ...
    Uri? gateUrl;
    AidokuCloudflareGate.resolver = (Uri url) async {
      gateUrl = url;
      return true;
    };

    final Map<String, Object?> details = await IosAidokuRuntime(jar: jar)
  ...
  test('non-https or missing challengeUrl never opens a resolver', () async {
    int solves = 0;
    installHost((_) async => throw PlatformException(
          code: 'CLOUDFLARE_CHALLENGE',
          message: 'blocked',
    ...
    await expectLater(
      runtime.search('/pkg.aix', query: 'x'),
      throwsA(isA<AidokuRuntimeException>()
          .having((AidokuRuntimeException e) => e.challengeUrl, 'url', isNull)),
    );
    expect(solves, 0);
    expect(requests.length, 1);
  });
