...
- **合并时补齐**：原修复只接了 3 个调用点，首页 overview 实际有**三条**横滚行
  （继续观看 / 下一集 / 最近添加），每条都可能出合集卡。`_buildNextEpisodeRow` 的
  `home_video_next_collection_$cid` 与 `_buildRecentlyAddedRow` 的
  `home_video_recent_collection_$cid` 两张**本地合集卡**仍然没有标签层——与原 bug
  同一形状（按卡型补而不是按「行 × 卡型」逐格补）。已各补 `tags: _collectionTags(cid)`，
  并在测试里各加一条断言；变异实测两处分别打死对应用例，还原后文件 SHA-256 一致。
  远端占位卡（`*_remote_*`）无本地条目、无标签，保持不传。
