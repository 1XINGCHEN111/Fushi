...
AppIconSelection publishAppIconSelection(AppIconSelection selection) {
...
/// 顺序保存并发布当前选择。通知只发生在偏好写入成功之后；自定义图标还会先逐出
/// 固定路径对应的旧 [FileImage]/[ResizeImage] 缓存，再用新 revision 强制图片组件
/// 建立新 stream。调用方在原生图标已切换而持久化失败时负责发布本次运行态。
Future<AppIconSelection> saveAppIconSelection(
  ...
  if (resolved.usesCustomFile) {
    final FileImage fileImage = FileImage(File(resolved.customPath!));
