...
void main() {
  ...
  group('in-app popup zero pollution', () {
    ...
    test('popup.js must NOT contain the host stack symbols', () {
      expect(
        popupJs.contains('renderStack'),
        isFalse,
        reason: 'renderStack lives only in global_lookup_host.js',
      );
      expect(
        popupJs.contains('__globalLookupHost'),
        isFalse,
  ...
  group('stack renderer calls renderStack (not bare renderPopup)', () {
    test(
      'buildStackRenderScript invokes window.__globalLookupHost.renderStack',
      () {
        ...
        expect(
          at,
          greaterThan(-1),
          reason: 'buildStackRenderScript must exist',
        );
    ...
    test(
      ...
      () {
        ...
        expect(
          controller.contains('window.renderPopup'),
          isFalse,
          reason:
        ...
        expect(
          controller.contains('_renderResult'),
          isFalse,
  ...
  group('P3c top-level host wiring', () {
    test('global_lookup_host.html links popup.css but NOT popup.js', () {
      ...
      expect(
        html.contains('popup.css'),
        isTrue,
        reason: 'the host document needs popup.css for the shell/card chrome',
      );
      expect(
    ...
    test('cpp navigates to global_lookup_host.html (not popup.html)', () {
      ...
      expect(
        ...
        isTrue,
        reason: 'the bare window top document must be the host, not popup.html',
      );
      expect(
        cpp.contains('Navigate(L"https://hibiki.popup/popup.html")'),
        isFalse,
    ...
    test('cpp loads + injects host.js at document start', () {
      final String cpp = read('windows/runner/global_lookup_window.cpp');
      expect(
        cpp.contains('LoadHostScript'),
        isTrue,
        reason: 'host.js is read from disk like the adapter (LoadHostScript)',
      );
      ...
      expect(
        h.contains('std::wstring LoadHostScript() const;'),
        isTrue,
  ...
  group('P3c nested-stack host wiring (C1/C3/E2/D2/E1)', () {
    ...
    test(
      'C1: host re-anchors a child onLinkClick rect + stamps the frame id',
      () {
        expect(
          hostJs.contains('function anchorRectToScreen('),
          isTrue,
          reason: 'the host converts a child LOCAL rect to window-local CSS px',
        );
        expect(hostJs.contains('function transformFrameMessage('), isTrue);
        expect(
          hostJs.contains('__frameId'),
          isTrue,
          reason: 'every bubbled message is stamped with its source frame id',
        );
        expect(
          hostJs.contains('function wrapFrameBridge('),
          isTrue,
    ...
    test('C3: host dismisses on a backdrop pointerdown / forwarded click', () {
      expect(
        hostJs.contains('function onHostPointerDown('),
        isTrue,
        reason: 'capture-phase pointerdown outside all shells dismisses root',
      );
      expect(
        hostJs.contains("postToHost('dismissPopupAt', [0])"),
        isTrue,
        reason: 'a click outside all shells dismisses the root (index 0)',
      );
      expect(
        hostJs.contains('function handleGlobalClick('),
        isTrue,
        reason: 'E2: C++ forwards a global click; the host hit-tests shells',
      );
      final String c = controller;
      expect(
        c.contains('_layerIndexForFrameId('),
        isTrue,
        reason: 'controller maps a stamped tapOutside to its layer index (C3)',
      );
      expect(
    ...
    test('C4/E2: the window-thread click handler no longer unconditionally '
      ...
      expect(
        cpp.contains('ForwardGlobalClickToHost'),
        isTrue,
        reason: 'a click inside the stack window is forwarded to the host',
      );
      expect(
        cpp.contains('handleGlobalClick('),
        isTrue,
      ...
      expect(
        hookBody.contains('ForwardGlobalClickToHost'),
        isTrue,
        reason: 'the handler forwards an in-window click instead of hiding it',
      );
      expect(
        hookBody.contains('Hide()'),
        isTrue,
    ...
    test('D2/E1: host reports a union bbox; Dart reveals the window to it', () {
      ...
      expect(
        channel.contains("_invoke<void>('revealStack'"),
        isTrue,
        reason: 'E1: a revealStack channel reveals/resizes to the bbox',
      );
      expect(
        cpp.contains('void GlobalLookupWindow::RevealStack('),
        isTrue,
        reason: 'native RevealStack positions + sizes the window to the bbox',
      );
    });
    ...
    test('TODO-1231 P1: __hasChildPopup rides its own channel, not the body', () {
      ...
      expect(
        hostJs.contains('function applyHasChildPopup('),
        isTrue,
        reason: 'the host applies __hasChildPopup on its own cheap channel',
      );
      expect(
        hostJs.contains('win.__hasChildPopup = desired;'),
        isTrue,
        reason:
            'applyHasChildPopup assigns the boolean directly in the frame realm',
      );
      // renderPayload must gate the full re-eval on a real body change.
      expect(
        hostJs.contains('record.injectedSettingsJs !== descriptor.settingsJs'),
        isTrue,
        reason: 'legacy cached renders retain their compatibility diff',
      );
      expect(
        hostJs.contains('record.injectedStaticRevision !== revision'),
        isTrue,
      ...
      expect(
        ...
        isTrue,
        reason: 'an unknown revision still sends a self-contained cold payload',
      );
    });

    test(
      'BUG-1833: close/back uses id-only retainStack, never full survivor payload',
      () {
        final int builderAt = render.indexOf('String buildRetainStackScript(');
        final int builderEnd = render.indexOf(
          'String buildHighlightFrameScript(',
          builderAt,
        );
        expect(builderAt, greaterThanOrEqualTo(0));
        expect(builderEnd, greaterThan(builderAt));
        final String builder = render.substring(builderAt, builderEnd);
        expect(builder, contains('__globalLookupHost.retainStack('));
        for (final String forbidden in <String>[
          ...
          expect(
            ...
            reason: '轻量截栈只允许传 frame id，禁止重新序列化 $forbidden',
          );
        }

        final int helperAt = controller.indexOf(
          'Future<void> _retainRenderedStack()',
        );
        final int helperEnd = controller.indexOf(
          'int? _firstIntArg(',
          helperAt,
        );
        expect(helperAt, greaterThanOrEqualTo(0));
        expect(helperEnd, greaterThan(helperAt));
        final String helper = controller.substring(helperAt, helperEnd);
        expect(helper, contains('buildRetainStackScript('));
        expect(helper, isNot(contains('_renderStack(')));
        expect(
        ...
        final int retainAt = hostJs.indexOf('function retainStack(');
        final int retainEnd = hostJs.indexOf(
          'function resetGeometryTransaction(',
          retainAt,
        );
        expect(retainAt, greaterThanOrEqualTo(0));
        expect(retainEnd, greaterThan(retainAt));
        final String retain = hostJs.substring(retainAt, retainEnd);
        expect(retain, contains('removeMissing(keepIds)'));
        expect(retain, contains('scheduleMeasure(activeRoute, true)'));
        expect(retain, isNot(contains('renderPayload(')));
        expect(retain, isNot(contains('injectContent(')));
        expect(retain, isNot(contains('renderStack(')));

        final int destroyAt = hostJs.indexOf('function destroyRecord(');
        final int destroyEnd = hostJs.indexOf(
          'function primeStandbyRecord(',
          destroyAt,
        );
        final String destroy = hostJs.substring(destroyAt, destroyEnd);
        expect(
          destroy,
          isNot(contains('resetParkedRealm(record)')),
          reason: '永久 iframe 销毁不得先执行昂贵的“复用 reset”',
        );
        final int removeAt = hostJs.indexOf('function removeMissing(');
        final int removeEnd = hostJs.indexOf('function retainStack(', removeAt);
        final String remove = hostJs.substring(removeAt, removeEnd);
        expect(remove, contains('parkCandidate'));
        expect(
          RegExp(r'parkRecord\(parkCandidate\)').allMatches(remove).length,
          1,
          reason: '单槽 warm pool 的整批删除只能 park 一次',
        );
      },
    );

    test(
      'BUG-1833: ancestor replacement skips the prefix-only intermediate render',
      () {
        final int lookupAt = controller.indexOf('Future<void> _lookupNested(');
        final int lookupEnd = controller.indexOf(
          'void _resetStackRoot(',
          lookupAt,
        );
        expect(lookupAt, greaterThanOrEqualTo(0));
        expect(lookupEnd, greaterThan(lookupAt));
        final String body = controller.substring(lookupAt, lookupEnd);
        expect(
          body,
          isNot(contains('Future.wait<void>')),
          reason: '查询成功前不得先提交 prefix-only 几何导致整栈闪一下',
        );
        expect(body, contains('if (childPushed) {'));
        expect(body, contains('await _renderStack();'));
        expect(body, contains('} else if (renderPrunedStack) {'));
        expect(body, contains('await _retainRenderedStack();'));
      },
    );
    ...
    test('TODO-1231 P2: layer shift is C++-ordered after SetWindowPos', () {
      ...
      expect(
        rsBody.contains('commitLayerShift'),
        isTrue,
      ...
      expect(
        rsBody.contains('if (!SetWindowPos('),
        isTrue,
        reason:
            'a failed HWND resize must return without acknowledging geometry',
      );
      expect(
        rsBody.contains('std::to_wstring(geometry_epoch)'),
        isTrue,
        reason: 'the successful commit carries the renderer transaction epoch',
      );
      // The bbox origin (CSS px) is carried to native so the host negates it.
      expect(
        channel.contains("'left': left"),
        isTrue,
    ...
    test('TODO-1231 (BUG-583): beginLookup re-gates WITHOUT observing the stale '
      ...
      expect(
        beginBody.contains("setAttribute(ATTR_CONTENT_READY, 'false')"),
        isTrue,
        reason:
      ...
      expect(
        renderBody.contains('} else if (hasContent(record)) {'),
        isTrue,
        reason:
    ...
    test('TODO-1231 (BUG-583): the overlay window origin is ratcheted '
      ...
      expect(
        layout.contains('RatchetedOverlayBox ratchetOverlayOrigin('),
        isTrue,
        reason: 'the outward-only origin ratchet is a pure layout helper',
      );
      expect(
        controller.contains('ratchetOverlayOrigin('),
        isTrue,
      ...
      expect(
        applyBody.contains('ratchetOverlayOrigin('),
        isTrue,
        reason: '_applyOverlayBox uses the ratchet to compute the window box',
      );
      expect(
      ...
      expect(
        controller.contains('_ratchetLeft = double.infinity'),
        isTrue,
    ...
    test('TODO-1231 v2 (BUG-583): the bbox MIN-corner (window origin) follows '
      ...
      expect(
        measureBody.contains('if (record.contentReady) {'),
        isTrue,
        reason:
      ...
      expect(
        measureBody.contains('minLeftAll') && measureBody.contains('minTopAll'),
        isTrue,
        reason:
    ...
    test('BUG-1833: nested reveal waits for the exact committed full geometry', () {
      ...
      // committed bounds contain the complete shell.
      expect(
        hostJs.contains('function shellCoveredByCommittedGeometry('),
        isTrue,
        reason: 'coverage includes all four shell edges, not only left/top',
      );
      expect(
        hostJs.contains('function maybeFlipRevealReady('),
        isTrue,
      ...
      expect(
        renderBody.contains('maybeFlipRevealReady(record, record.route)'),
        isTrue,
      ...
      expect(
        commitBody.contains('maybeFlipRevealReady(record)'),
        isTrue,
        reason: 'matching commit re-checks held shells so they appear complete',
      );
      expect(
        commitBody.contains('epoch !== announcedGeometryEpoch'),
        isTrue,
        reason: 'a stale native epoch cannot reveal the replacement child',
      );
      expect(
        hostJs.contains('setGateFlag(record, ATTR_REVEAL_READY,') &&
            hostJs.contains('scheduleMeasure(route);'),
        isTrue,
        reason:
            'recovery may re-measure but does not force a nested reveal through '
            'an uncommitted geometry',
      );
    ...
    test('TODO-1345 (BUG-583 deeper): a reserved origin FLOOR freezes the window '
      ...
      final String layout = read('lib/src/lookup/global_lookup_layout.dart');
      expect(
        layout.contains('}) computeCascadeHeadroomSeed('),
        isTrue,
        reason: 'the screen-edge-aware headroom seed is a pure layout helper',
      );
      // The controller computes the floor from the real work area + pushes it.
      expect(
        controller.contains('computeCascadeHeadroomSeed('),
        isTrue,
      ...
      expect(
        render.contains("payloadObj['originFloor']"),
        isTrue,
        reason: 'buildStackRenderScript carries the origin floor to the host',
      );
      // host.js applies the floor as an OUTWARD-only lower bound on the origin.
      expect(
        hostJs.contains('function applyOriginFloor('),
        isTrue,
    ...
    test('D1: two-flag reveal gate hides a shell until content + geometry', () {
      ...
      // style write.
      expect(
        hostJs.contains("var ATTR_CONTENT_READY = 'data-content-ready';"),
        isTrue,
        reason: 'content-ready flag attribute is named + explicit',
      );
      expect(
        hostJs.contains("var ATTR_REVEAL_READY = 'data-reveal-ready';"),
        isTrue,
        reason: 'reveal-ready flag attribute is named + explicit',
      );
      expect(
        hostJs.contains('function ensureStyle('),
        isTrue,
        reason: 'host injects the reveal-gate stylesheet (it owns the shell)',
      );
      expect(
        hostJs.contains('.global-lookup-frame-shell{visibility:hidden'),
        isTrue,
      ...
      expect(
        hostJs.contains('window.MutationObserver'),
        isTrue,
        reason: 'content-ready uses a MutationObserver on contentDocument',
      );
      // Active shell visibility remains owned by the two data-* CSS gates. The
      // one off-screen standby realm is intentionally inline-hidden while
      // parked, and acquisition must clear that parking override before the
      // record becomes an active shell.
      expect(
        hostJs.contains("record.shell.style.visibility = '';"),
        isTrue,
        reason:
            'acquiring a parked realm restores declarative active-shell gating',
      );
      expect(
        hostJs.contains("record.shell.style.visibility = 'hidden';"),
        isTrue,
        reason: 'only the parked standby realm is inline-hidden',
      );
      expect(
        hostJs.contains('function scheduleMeasure('),
        isTrue,
  ...
  group('JS harness (node) — renderStack diff + reveal gate', () {
    test('global_lookup_host_test.mjs executes host.js end-to-end', () async {
      ...
      expect(
        ...
        contains('global_lookup_host_test: PASS'),
      );
    });
  });
  ...
  group('host.js iframe bridge contract', () {
    ...
    test('host iframes carry NO bridge-killing sandbox attribute', () {
      ...
      expect(
        hostJs.contains("setAttribute('sandbox'"),
        isFalse,
        reason: 'no sandbox on the same-origin host iframes (bridge contract)',
      );
      expect(
        hostJs.contains('sandbox ='),
        isFalse,
    ...
    test(
      'host iframes load popup.html (same-origin) for per-frame injection',
      () {
        expect(
    ...
    test(
      'host.js is the ONLY place renderStack/frames live (not popup.js)',
      () {
