## BUG-1787 · SGRE 查词 detour 丢失布局参数导致游戏控制码不转义
...
- **[x] ① 已修复** — `native/galgame_hook/hook/adapters/sgre_lookup.inc:221` 完全停止 detour UserHook1，改在游戏解析完成后的 draw 边界旁路复制字形；原函数只按原始 `this` 调用，查词不再修改或重放游戏文本（本提交）。
